...
public class Map {
    ...
    private static final boolean[] terrainOperational = new boolean[]{false,false,true,true,true,true,false};
    ...
    public Map(String mapType) {
        ...
        noiseMatrix = NoiseGenerator.generateNoiseMatrix(width, height, 70, 5, 0.5D, 1.7D);
            ...
            for (int j = 0; j < noiseMatrix[0].length; ++j) {
                for (int h = 0; h < terrainHeights.length; ++h) {
                    if (noiseMatrix[i][j] <= terrainHeights[h]) {
                        if(terrainOperational[h])
                            mapTilesMatrix[i][j]=new MapTile();
                        break;
                    }
                }
            }
